...
export enum Tasks {
    ...
    CHATGPT35 = "gpt-35-turbo-16k-config",
    CHATFALCON = "tiiuae--falcon-40b-instruct-config"
}
...
export const getChatCompletion = (task: Tasks = Tasks.CHAT)  => {
...
const chatCompletion = async (
    ...
    const resourceGroupId = tenant && tenant !== "main" ? `${tenant}-${getAppName()}` : `default-${getAppName()}`;
    const deploymentId = await getDeploymentId(resourceGroupId, task);
    console.log("########### model used:", task);
    if (deploymentId) {
        const aiCoreService = await cds.connect.to(AI_CORE_DESTINATION);
        const payload: any = {
            ...
            frequency_penalty: 0,
            presence_penalty: 0,
            model: task == Tasks.CHATFALCON ? 'tiiuae--falcon-40b-instruct' : undefined,
            stop: "null",
            ...LLMParams
        };
        const headers = { "Content-Type": "application/json", "AI-Resource-Group": resourceGroupId };
...
export const getResourceGroups = async (): Promise<Array<any>> => {
    try {
        const response = await ResourceGroupApi.kubesubmitV4ResourcegroupsGetAll()
            .skipCsrfTokenFetching()
            .execute(aiCoreDestination);
        console.log("AAAAAAA - getResourceGroups ", response.resources);
...
export const createConfigurations = async (configuration: ConfigurationBaseData, headers: AICoreApiHeaders): Promise<Array<any>> => {
    console.log("CCCCCCCCC - createConfigurations")
    const responseConfigurationCreation = Promise.all(
        CONFIGURATIONS.map((config : any) => {
            return ConfigurationApi.configurationCreate({
                ...
                name: config.name,
                // @ts-ignore
                executableId: config.executableId ? config.executableId : EXECUTABLE_ID,
                // @ts-ignore
